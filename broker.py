...
def is_generated_client(content: str) -> bool:
    first_lines = content.splitlines()[:3]
    return any("Generated by android-build-broker v1." in line for line in first_lines)
...
def init_repo(args: argparse.Namespace) -> int:
    ...
    if client_path.exists():
        existing = client_path.read_text(encoding="utf-8")
        if existing != desired and not is_generated_client(existing):
